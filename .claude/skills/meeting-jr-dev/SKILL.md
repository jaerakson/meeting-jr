...
### 소환 직후 — 이름 확정
- **팀을 재구성했으면 `ListAgents` 로 실제 이름을 먼저 확정한다.**
- 같은 이름의 죽은 인스턴스가 남아 있으면 새 팀원에게 `-2` 같은 접미사가 붙는다.
  이때 **원래 이름으로 보낸 SendMessage 는 죽은 인박스로 가면서도 `success: true` 를 반환한다.**
  실패로 표시되지 않아 양쪽이 조용히 유실된 채 대기만 한다(세션 56에서 실제 발생, 팀리드·director 양쪽에서 관측).
- 죽은 팀원은 즉시 TaskStop 으로 정리하고, 팀리드는 소환 직후 정확한 이름을 팀에 통보한다.
