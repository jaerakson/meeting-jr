...
| **[후속 과제]** 프론트가 전송용 페이로드와 화면용 렌더를 분리하지 않는다 (front-c3 전수조사) | **근본 원인**: `Transcript.tsx` 의 `onTranscriptChange` 가 항상 `render(segments, speakerMap)`(**이름 적용판**)을 콜백으로 내보낸다. 따라서 편집 세션 중 **화자 이름을 한 번이라도 바꾸면 그 시점부터** `speakerMap` 이 채워져, 이후 **텍스트만 고치는 `saveEdit` 조차 이름이 구워진 문자열**을 만든다. 이것이 `MainArea` 의 `handleSaveTranscript`(→`PATCH /transcript`)와 `handleResummarize`(→`POST /finalize`) **두 결함의 공통 근원**이다. **현재는 서버가 두 경로 모두 `restore_segment_labels` 로 사후 역복원해 막고 있다.** 프론트에서 근원을 닫으려면 **라벨 기준 문자열을 별도로 유지해 콜백에 넘기면** 된다. **다만 서버측 방어는 프론트를 고쳐도 계속 필요하다** — 구버전 번들·다른 클라이언트·직접 API 호출에 무방비이므로, 프론트 수정은 서버 방어의 *대체*가 아니라 *추가 방어*다. **참고**: `TranscriptEditor` 는 `render(segments, {})` 로 **빈 map 을 넘겨 `display == label` 을 강제**하므로 이 부류가 아니다(같은 `render` 호출이라도 인자로 안전/위험이 갈린다). |
