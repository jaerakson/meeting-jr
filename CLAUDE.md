...
1. `PROGRESS.md` 최상단 1~3개 항목 — 직전 작업과 다음 할 일
2. **`.claude/skills/meeting-jr-dev/SKILL.md` 의 "운영 규칙" 절** — 커밋·코드프리즈·리뷰 강도·테스트 조건·검증 원칙.
   세션 52~55에서 실제 사고를 겪고 만든 규칙이다. 이걸 건너뛰면 같은 사고가 반복된다.
3. `docs/ai_analysis/20260828_잔여_기획_후보.md` — 미승인 기획 후보와 후속 과제
4. (배경이 필요하면) `docs/ai_analysis/20260829_업무_처리_방식.md` — 위 규칙이 왜 생겼는지

> **다음 작업 1순위는 화자 매핑 리팩터링이다.** 화자 이름을 문자열 토큰이 아니라
> 라벨 기준으로 다루도록 바꾼다. 이 영역에서 같은 부류 버그가 5라운드 연속 나왔고
> (PR #78·#79), 폐기된 매칭 방식이 두 번 재유입됐다. 패치로는 끝나지 않는다.
